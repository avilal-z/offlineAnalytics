...
/**
The object is used as a wrapper for basic types like uchar, uint, float or double ...
...
/**
The matrix element is used to carry multi, or single channel types, like XYZ for the PointCloud
The interface does not carry any data, just the information about the type
*/
...
#pragma pack(push, 1)
///3 Dimensional Point or Vector of a specified ChannelType - 3 channel Matrix element
...
General n-dimensional Matrix interface
*/
...
class MatInterface {
  ...
  protected:
    ...
    */
    ...
    ///Reshape the Matrix to be n-dimensional SizeND[0], SizeND[1], ... SizeND[n-1] and reallocate data if needed
...
class Mat2D : public MatInterface<ElementType, 2> {
  ...
  public:
    ...
    */
    ...
    ///Reshape the matrix to be 2 dimensional Size.Height x Size.Width and reallocate data if needed
    ...
    You can choose of multiple types of DataOwnershipTransfer
        ...
        NoCopyNoOwnership - Data will reference to remote pointer and the data will not be freed in the object destructor
        NoCopyFullOwnership - Data will reference to remote pointer and the data will be copied in the object destructor
    */
...
class Frame {
  public:
    ...
    FrameInfo Info;
    ///2 dimensional organized 3 channel PointCloud structure, organized as [X mm, Y mm, Z mm] of 32 bit floats, unmeasured point has [0.0, 0.0, 0.0]
    PointCloud32f PointCloud;
    ///2 dimensional organized 3 channel NormalMap structure, organized as normalized [X, Y, Z] of 32 bit floats, unmeasured normal has [0.0, 0.0, 0.0]
    NormalMap32f NormalMap;
    ///2 dimensional 1 channel Depth map of 32 bit floats coding orthogonal distances from the internal camera in mm
    DepthMap32f DepthMap;
    ///2 dimensional 1 channel Confidence map of 32 bit floats coding measurement confidence
    ...
    ///Returns resolution of the captured frame - All outputs are either empty, or have this resolution
    PhoXiSize PHOXI_DLL_API GetResolution() const;
    ///Creates a shared_ptr of Frame with new allocated data in the local address space
    ...
    /**
    Saves Frame structure as an organized Stanford's PLY with specified Texture and Normals.
    All informations are stored, this can be used as an container.
    @return true on success.
    @param FilePath Input ply FilePath
    @param BinaryFile If true, the ply will be stored in binary form, more memory efficient (true is default)
    @param NormalizeTexture If true, the RGB information in the PLY will be normalized, 32 bit Intensity information will be untouched
    @param StorePointCloud If true and PointCloud is not empty, the PointCloud will be stored into PLY
    @param StoreNormalMap If true and NormalMap is not empty, the NormalMap will be stored into PLY
    @param StoreDepthMap If true and DepthMap is not empty, the DepthMap will be stored into PLY
    @param StoreTexture If true and Texture is not empty, the Texture will be stored into PLY
    @param StoreConfidenceMap If true and ConfidenceMap is not empty, the ConfidenceMap will be stored into PLY
    @param Unordered If true the Frame will be stored without non-measured points
    */
    bool PHOXI_DLL_API SaveAsPly(const std::string &FilePath,
    ...
    */
    bool PHOXI_DLL_API SaveAsPly(const std::wstring &FilePath,
      bool BinaryFile = true,
      bool NormalizeTexture = true,
      bool StorePointCloud = true,
      bool StoreNormalMap = true,
      ...
      const std::vector<float> & cameraBinning = std::vector<float>()) const;

...
/**
PhoXiFeatureSetGet<T> is a Temporary container for the Value of PhoXiFeature<T>.
    It allows the Value of PhoXiFeature<T> to be referenced by -> and after the PhoXiFeatureSetGet<T> goes out of scope,
    it call SetValue() on PhoXiFeature<T> if the content of PhoXiFeatureSetGet<T> was modified
*/
template<typename T>
...
class PHOXI_DLL_API PhoXiFeatureInterface {
  public:
    ...
    /**
    Some features can be Hardware dependent and could be present only for specific families of products
    If the feature is not implemented for a Device Type, no instance of the family will have the feature
    @return true if the feature is implemented for the specific device type, returns false otherwise
    */
    bool isImplemented() const;
    ///Tells if specific feature is enabled for this camera
    /**
    Some features can be implemented for a specific Device class, but are not enabled because of a minor difference between the instances of the same family
    @return true if the feature is enabled for the specific device, returns false otherwise
    */
    bool isEnabled() const;
    ///Returns last error message from last assign or read of the feature value
    /**
    call to GetLastErrorMessage can also be used to determine the error state. If the returned string is empty, there was no error
    @return last error message or "" (empty string) if the last operation was successful
    */
  ...
  protected:
    ...
    bool Initiated;
    //If the DefaultControl is true, the Feature will directly get/set the this->Value while Feature is Implemented, Enabled, Gettable/Settable and PhoXi is not Connected.
    bool DefaultControl;
...
class PHOXI_DLL_API PhoXiFeature : public PhoXiFeatureInterface {
  ...
  public:
    ...
    /**
    Some features does always communicates with the device Hardware, while others do cache the data locally
    @return Locally Stored Value
    */
    inline T GetStoredValue() const;
    ...
    For Example, if it is desired to change the ShutterMultiplier:
        ...
        PhoXiDevice->CapturingSettings->ShutterMultiplier = NewValue; --> this will translate to similar behaviour as described before automatically
    */

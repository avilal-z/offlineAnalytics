...
/**
Settings inside the structure affect the data size of the output and will rebuild the Cyclic buffer
To get supported capturing modes, call SupportedCapturingModes
...
class PHOXI_DLL_API PhoXiCapturingMode {
  public:
    ...
    The Resolution will affect the Hardware of the sensor, with lower resolution, you can increase the S/N ratio and improve the per point scanning quality and speed if high resolution is not a necessity
...
/**
Currently, only Freerun and Software Trigger are supported
The type is also used as a smart enumerator
...
/**
PhoXiTimeout is used in all calls when Timeout have sense
    calls Like GetFrame takes Timeout as optional parameters to wait for the frame
        If default param is supplied, timeout supplied as PhoXiDevice->Timeout will be used
*/
...
Currently, only Freerun and Software Trigger are supported
The type is also used as a smart enumerator
PhoXiDevice->CoordinateSettings->CoordinateSpace = PhoXiCoordinateSpace::CameraSpace
*/
...
class PHOXI_DLL_API PhoXiCoordinatesSettings {
  public:
    ...
    ///Defines if the Recognize Markers feature is activated. If this feature is activated - It will try to recognize Photoneo's Marker field.
...
All chosen outputs needs to be transported from the Device and affect latency
PointCloud and DepthMap use shared structure, Both selected had the same effect (speaking of data package) as one or the other
*/
...
Coding strategy optimized for Interreflections use advance digital coding to make diffuse inter-reflection possible to suppress
...
/**
Select a desired Coding Quality
This parameter influences the scanning time
Default is High
*/
...
/**
Select a source of the texture
Computed is a texture computed from the structured patterns - no additional image is necessary -> Lower Quality
LED will trigger an additional image with LED flash that offers an speckle less 2D image
Laser will trigger an additional image with Laser flash
...
/**
Defines data cutting volumes in either Camera, or PointCloud space
Cutting the data based on 3D component in CameraSpace helps cut depending on camera coordinate space,
while ROI defined in PointCloud Space can be used for cut only data inside a specific box, or on top of Marker Space ...
The filtration will only be applied if max > min and is applied individually for any dimension
e.g. you can specify CameraSpace.min.z to 500 and CameraSpace.max.z to 1000 to filter all data that are outside of this z region
*/
...
#pragma pack(1)
///Defines data cutting according to a specific threshold value of angle between point normal and specified vector
/**
Defines data cutting according to a specific threshold value of angle (in degrees) between point normal and specified vector
Specifically useful to cut out parts of the surface that are too diverted from the Camera, or Projector
*/
class PHOXI_DLL_API PhoXiNormalAngle {
  public:
    ///Maximal angle between point normal and direction to camera, default 90 degrees
    double MaxCameraAngle;
    ///Maximal angle between point normal and direction to projector, default 90 degrees
    double MaxProjectorAngle;
    ///Minimal angle between normal and halfway vector, default 0 degrees
    double MinHalfwayAngle;
    ///Maximal angle between normal and halfway vector, default 0 degrees
...
Normal -> standart sensor setting best for most scanes
...
#pragma pack(1)
///Capturing Settings are settings that affect the data capturing phase of the scanning
/**
...
class PHOXI_DLL_API PhoXiCapturingSettings {
  public:
    ...
    /**
    With Shutter Multiplier, you can control the time of individual patterns of the scanner
    ...
    PhoXi devices uses Scanned laser to project patterns, ShutterMultiplier does not affect the time of individual sweep, it rather tells the number of repeats of individual pattern
    -   It affects the scanning process and the final Scanning Time is multiplied by ShutterMultiplier
    */
    int ShutterMultiplier;
    ///Defines the number of scans that will be taken and merged to single output
    /**
    With Scan Multiplier, you can scan multiple times and merge the output to a single 3D cloud,
    This will increase the S/N ratio to ensure higher quality scan
    It is useful in scenarios with High Dynamic Range, where will increased ShutterMultiplier means over saturation of some areas of the scanning
    -   It affects the scanning process and the final Scanning Time is multiplied by ScanMultiplier
    */
    int ScanMultiplier;
    ///Specify a mode, where only the camera captures the frame
    /**
    With Camera Only Mode, you can use the scanner internal camera to capture only 2D images of the scene. This images then can be read out as Texture.
    ...
    */
    ...
    /**
    Ambient light suppression samples the scene multiple times during one pattern exposure.
    ...
    -   Enabling the mode will fix Shutter multiplier to the value of 2
    */
    bool AmbientLightSuppression;
    ///Specifies maximum fps
    /**
    Useful for limiting the fps in freerun mode
    If you want to allow a scan every 10 seconds, you can set MaximumFPS to 0.1. If you want 2 scans per second, set the value to 2.0 
    0 is default - unlimited
    */
    double MaximumFPS;
    ///Defines used single pattern exposure
    double SinglePatternExposure;
    ///Defines used coding strategy
    PhoXiCodingStrategy CodingStrategy;
    ///Defines used quality of coding
    PhoXiCodingQuality CodingQuality;
    ///Defines source of Texture Frame
    PhoXiTextureSource TextureSource;
    ///Defines used Laser Power
    int LaserPower;
...
class PHOXI_DLL_API PhoXiProcessingSettings {
  public:
    ...
    ///Estimates the radius of normal estimation
    /**
    higher values (4 max -> 9x9 neighborhood) offers smoother normals, while smaller values (1, 2) is good for retrieving smaller depth details
    Affects normal map generation
    */
...
#pragma pack(1)
///Used as output of calibration of additional camera and input for computation of aligned depth map
